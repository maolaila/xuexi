...
AST是一个概念，作用是通过词法分析、句法分析等方式，将一套语法转换成另一套语法；

词法分析：词法分析指的是将对象逐个扫描，获取每一个字母的信息，生成由对象组成的一维数组。

句法分析：语法分析指的是将有关联的对象整合成树形结构的表达形式。

![ast-tree](C:\Users\maolaila\Desktop\面试题\Interviewer-master\ast-tree.webp)

> 在最外层的 type、start、end、body、sourceType 中，我们主要看中间 body 部分。把每一个 body 当成一个块来拆解。

> 第一个 body（块） 就是 function exp(a) 这段。这是一个 type 为 FunctionDeclaration （函数定义）类型的对象。每一个函数定义的块中都有一个 id 对象。如上所示，它的 id 对象信息中 type 为 Identifier，name 为 exp。它不是表达式，也不是异步。所以 express、generator、async 皆为 false。最后它有一个参数 params 对象集，在该参数对象集中参数名 name 为 a。

> 再看第二个 body（块） 就是 return a。这是一个 type 为 BlockStatement （块语句）类型的对象。在其内部有一个 type 为 ReturnStatement （返回语句）类型的语句。在返回的 argument 集合中参数名 name 为形参 a。
...
React可以作为MVVM中第二个V，也就是View，但是**并不是MVVM框架**。MVVM一个最显著的特征：双向绑定。React没有这个，它是单向数据绑定的。React是一个单向数据流的库，状态驱动视图。react整体是函数式的思想，把组件设计成纯组件，状态和逻辑通过参数传入，所以在react中，是单向数据流，推崇结合immutable来实现数据不可变。
...
-   4.  也可以new一个 Vue 的 EventBus,进行事件监听，一边执行监听，一边执行新增 VUE的eventBus 就是**发布订阅模式**，是可以在React中使用的;
...
1. 对比当前新旧虚拟DOM，在对比过程中直接更新真实DOM，边对比，变更新；
2. 只在同层级对比；
3. 时间复杂度是O(n^3)；原因：不管新旧虚拟DOM，它们的结构都是树形结构，对比两棵树变化的算法时间复杂度是O(n^3)；
...
合成事件指的是react用js模拟了一个Dom事件流。（fiber树模拟Dom树结构） 合成事件的事件流在fiber树中发生捕获和冒泡。

**从点击输入框开始** 

当你点击input输入框，react在根节点*（注：根节点，在 react-v16 为 document ，在 react-v17 为挂载容器Dom ）*监听到focus事件*（注：focus事件并不是冒泡事件，react对非冒泡事件在捕获阶段监听）（注：根节点对所有事件进行了监听，除了特别例外submit、reset、invalid和媒体事件等）*。

> 如何从原生事件找到对应的虚拟Dom？

此时，react得到的信息只有原生事件对象（nativeEvent）。react通过nativeEvent对应的Dom（eventTarget），沿着Dom树向上找到距离该eventTarget最近的被react管理的Dom节点（*注：react将fiber树挂载到Dom树上时，每个宿主（host）类型fiber节点与Dom节点一一对应，并链接）（注：向上查找是因为可能子Dom节点并不是被react管理的，如第三库滚动插件等）*，并获得对应的fiber A。
