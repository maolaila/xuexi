...
JSX和Fiber都是React的核心概念，但是它们的职责不同。

JSX是一种语法扩展，它允许我们在JavaScript中编写类似于HTML的代码。这使得编写React组件更加直观和易读。在编译时，JSX代码将被转换成React元素的函数调用，这些元素将被用于构建React组件的虚拟DOM树。

Fiber是React 16中的一种新的协调引擎，它被设计用于实现增量渲染和调度。Fiber可以将渲染过程分成一系列小的任务，这些任务可以被中断、暂停、恢复和重新启动。这使得React能够更好地响应用户的交互和动画，提高了应用的性能和用户体验。

虽然JSX和Fiber都是React的核心概念，但是它们的职责不同。JSX主要是一种语法扩展，用于编写React组件的代码；而Fiber则是一种新的协调引擎，用于实现React的增量渲染和调度。
...
合成事件的理解：合成事件是React中用于处理用户交互的一种机制，它是基于事件委托（event delegation）实现的。事件委托是指将事件绑定到一个共同的父节点上，而不是将事件绑定到每个子节点上，从而减少事件绑定的次数，提高性能。
